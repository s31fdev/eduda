...
История ведётся у двух статусов — записи «ученик — группа» (`StudentGroup.status`) и урока (`Lesson.status`). Таблица одна, `StatusChange`, append-only, как `WalletEntry`: строки не правятся и не удаляются. Колонки `status` / `statusChangedAt` / `statusComment` остаются кешем поверх неё — по ним идут отбор и сортировка списков.

**Все смены проходят через `src/features/status-log/record.server.ts`**: `setStudentGroupStatusTx` (запись есть — обновляет, нет — создаёт), `removeStudentGroupTx` (удаление из группы) и `setLessonStatusTx`. Каждая переписывает колонку и пишет строку журнала вместе, со снимком имени группы и автором. **Не меняйте `StudentGroup.status` и `Lesson.status` мимо них** — журнал отстанет молча. Закрытие группы (`closeStudentGroupsTx`) идёт через ту же дверь, по одной записи: в `updateMany` теряется прежний статус каждой. `addStudentToGroup` поэтому отказывает на существующей записи сам — раньше это делал первичный ключ, а дверь такую запись молча переписала бы.
...
- **Цепочка непрерывна:** `fromStatus` = `toStatus` предыдущей строки; первая строка записи и строка после `REMOVED` начинаются с `null`. `REMOVED` — псевдостатус удалённой записи, в `StudentStatus` его нет; поэтому `toStatus` строкой, а не енумом.
