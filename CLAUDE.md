...
- **Это единственная операция, которая двигает выручку закрытых месяцев.** Прошедшие занятия переоцениваются парой строк журнала на дне занятия — `REVERSAL` по старой цене со ссылкой на списание и новый `CHARGE` по новой, на кошельке списания (`repriceChargedTx`). Уроков пара не двигает, старые строки не правятся. Окно называет сдвиг выручки до подтверждения, история хранит его в `meta.pastRevenueDelta`. Обычные оплаты закрытые месяцы по-прежнему не двигают. Налог за прошлый месяц в «Прибыли» пересчитается вместе с выручкой — если его уже заплатили, отчёт разойдётся с уплаченным.
