...
**Откат смотрит в журнал, а не на цену.** У занятий, закрытых нулём при переходе, цена стоит, а движения денег за ними нет: это разовые визиты, которым платить было нечем (`close-unbillable-attendances.ts`), и прощённые отработки (`forgive-missed-makeups.ts`) — 133 строки на проде. Возврат на такой строке выдал бы урок из воздуха и оставил бы в журнале откат без своей пары, поэтому `unchargeAttendanceTx` сначала ищет непогашенное списание в журнале и, не найдя, снимает только цену. Проверять по `packageId` для этого нельзя: у занятий, списанных «в долг» до перехода, пакета тоже нет, но баланс они двигали. Правило держится на инварианте «Σ журнала = баланс» и покрывает ровно эти 133 строки — у остальных 18 064 строк с ценой списание в журнале есть.
